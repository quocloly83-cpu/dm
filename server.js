...
async function readGithubStore() {
  ...
  if (!owner || !repo) {
    throw new Error("GITHUB_REPO không hợp lệ");
  }

  const safePath = String(GITHUB_DATA_PATH || "keys.json")
    .split("/")
    .filter(Boolean)
    .map(encodeURIComponent)
    .join("/");
  const apiPath = `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${safePath}?ref=${encodeURIComponent(GITHUB_BRANCH)}`;

  const file = await githubRequest("GET", apiPath);
  if (!file || typeof file.content !== "string") {
    throw new Error("GitHub không trả về nội dung keys.json");
  }
...
async function writeGithubStore(store) {
  ...
  const safePath = String(GITHUB_DATA_PATH || "keys.json")
    ...
    .map(encodeURIComponent)
    .join("/");
  const apiPath = `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${safePath}`;
...
function normalizeKeyItem(item) {
  ...
  let totalDevices = Number(out.totalDevices);
  if (!Number.isFinite(totalDevices) || totalDevices < 0) {
    if (Number.isFinite(Number(out.maxDevices))) totalDevices = Number(out.maxDevices);
    else if (Number.isFinite(Number(out.uses))) totalDevices = Number(out.uses);
    else totalDevices = out.devices.length;
  }

  let usesLeft = Number(out.usesLeft);
  if (!Number.isFinite(usesLeft) || usesLeft < 0) {
    if (Number.isFinite(Number(out.uses))) usesLeft = Number(out.uses);
    else if (Number.isFinite(Number(out.maxDevices))) {
      usesLeft = Number(out.maxDevices) - out.devices.length;
    } else {
      usesLeft = Math.max(0, totalDevices - out.devices.length);
    }
  }

  usesLeft = Math.max(0, usesLeft);
  totalDevices = Math.max(out.devices.length, totalDevices, out.devices.length + usesLeft);

  const expireAt = Number(out.expireAt || out.expiresAt || 0);
  const createdAt = Number(out.createdAt || Date.now());
  ...
  return {
    ...
    usesLeft,
    totalDevices,
    expireAt: Number.isFinite(expireAt) ? expireAt : 0,
...
async function initStore() {
  try {
    if (hasGithubStore()) {
      ...
      saveLocalStore();
      console.log("Store ready: GitHub", Object.keys(keys).length);
    } else {
      keys = normalizeAllStore(loadLocalStore());
      saveLocalStore();
      console.log("Store ready: local file", Object.keys(keys).length);
    }
  } catch (err) {
...
async function persistStoreSnapshot(snapshot) {
  try {
    ...
    if (hasGithubStore()) {
      const hasRemoteData = Object.keys(keys || {}).length > 0;
      const hasSnapshotData = Object.keys(snapshot || {}).length > 0;
      if (!hasSnapshotData && hasRemoteData) {
        throw new Error("Refuse to overwrite remote store with empty snapshot");
      }
...
function baseStyles() {
  ...
  <style>
    ...
    *{font-family:"Alata", Arial, sans-serif}
    ...
    .loginHero{
      position:relative;
      padding:34px 0 14px;border-radius:0;
    ...
    .loginIntro{position:relative;z-index:1;margin-bottom:22px}
    .loginTitle{font-size:34px;color:#fff7db;margin:0 0 10px;letter-spacing:.42px;text-shadow:0 0 22px rgba(255,224,130,.24),0 0 46px rgba(255,255,255,.09)}
    .loginHint{margin:0;color:#ddd2b2;font-size:14px;line-height:1.75;max-width:560px}
    ...
    .loginFrame{
      position:relative;padding:18px 14px 10px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;
      width:min(100%,520px);margin:0 auto;animation:floatPanel 4.8s ease-in-out infinite;
    }
    .loginFrame::before{
    ...
    .input{
      width:100%;height:64px;border:none;outline:none;border-radius:22px;padding:0 18px;
      color:#fff;background:linear-gradient(180deg, rgba(255,255,255,.055), rgba(255,255,255,.02));border:1px solid rgba(255,240,182,.14);font-size:15px;
    ...
    .btn{
      width:auto;min-width:138px;height:52px;margin-top:16px;padding:0 24px;
    ...
    .loginIconRow{display:flex;align-items:center;justify-content:center;gap:68px;margin-top:18px}
    ...
    @media (max-width:560px){
      ...
      .loginHero{padding:24px 0 8px}
      .loginFrame{width:min(100%,100%);padding:22px 10px 10px}
  ...
  <div class="wrap">
    <div class="card">
      ...
      <div class="content">
        ...
        <div id="loginView" class="loginHero" style="padding:2px 0 0;background:transparent;border:none;border-radius:0;box-shadow:none;min-height:auto;display:flex;align-items:flex-start;justify-content:flex-start">
...
app.get("/healthz", async (req, res) => {
  const base = { ok: true, storeReady, storeMode: getStoreMode(), repo: GITHUB_REPO, branch: GITHUB_BRANCH, path: GITHUB_DATA_PATH, keyCount: Object.keys(keys || {}).length };
  if (!hasGithubStore()) return res.json(base);
  try {
    const remote = await readGithubStore();
    return res.json({ ...base, remoteKeyCount: Object.keys(remote || {}).length });
  } catch (err) {
    return res.status(500).json({ ...base, ok: false, githubError: err.message, githubStatus: err.statusCode || 0 });
  }
});
...
app.post("/api/check", async (req, res) => {
  ...
  if (!alreadyUsed) {
    if (item.usesLeft <= 0) {
      return res.json({ ok: false, msg: "Key đã hết lượt nhập, bạn hóng key nhanh hơn ở video sau nhoa , admin chỉ rep tin nhắn khi mua files hoặc key vĩnh viễn nên đừng nhắn xin key nhé =))" });
    }
