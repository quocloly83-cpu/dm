    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          console.log(`[GitHub ${method}] ${apiPath} -> ${res.statusCode}`);
...
async function readGithubStore(options = {}) {
  ...
  try {
    ...
    if (err.statusCode === 404) {
      console.warn(`GitHub store not found at ${GITHUB_REPO}/${GITHUB_DATA_PATH} (${GITHUB_BRANCH})`);
      return {};
    }
    if (options.silent) {
      console.warn("GitHub read failed:", err.message);
      return null;
    }
    throw err;
  }
...
async function initStore() {
  try {
    if (hasGithubStore()) {
      ...
      fs.writeFileSync(STORE_PATH, JSON.stringify(keys, null, 2), "utf8");
      console.log(`Store ready: GitHub (${countStoreItems(keys)} keys)`);
    } else {
      keys = normalizeAllStore(loadLocalStore());
      saveLocalStore();
      console.log(`Store ready: local file (${countStoreItems(keys)} keys)`);
    }
  } catch (err) {
...
async function persistStoreSnapshot(snapshot) {
  const normalizedSnapshot = normalizeAllStore(snapshot);
  const snapshotCount = countStoreItems(normalizedSnapshot);

  try {
    if (hasGithubStore()) {
      const remoteStore = await readGithubStore({ silent: true });
      const remoteCount = countStoreItems(remoteStore || {});

      if (snapshotCount === 0 && remoteCount > 0) {
        throw new Error("Refuse to overwrite non-empty GitHub store with empty snapshot");
      }

      await writeGithubStore(normalizedSnapshot);
      fs.writeFileSync(STORE_PATH, JSON.stringify(normalizedSnapshot, null, 2), "utf8");
    } else {
      fs.writeFileSync(STORE_PATH, JSON.stringify(normalizedSnapshot, null, 2), "utf8");
    }
  } catch (err) {
...
async function saveStore(options = {}) {
  ...
  console.log(`Save store requested (${countStoreItems(snapshot)} keys) -> ${getStoreMode()}`);
  saveQueue = saveQueue
