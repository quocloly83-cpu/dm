...
function githubRequest(method, apiPath, body) {
  return new Promise((resolve, reject) => {
    ...
    console.log("👉 TOKEN PREFIX:", GITHUB_TOKEN ? GITHUB_TOKEN.slice(0, 10) : "(empty)");
...
async function readGithubStore() {
  ...
  console.log("PATH:", GITHUB_DATA_PATH);
  const [owner, repo] = String(GITHUB_REPO || "").split("/");
  if (!owner || !repo) throw new Error("GITHUB_REPO không hợp lệ");

  const apiPath = `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${GITHUB_DATA_PATH
    .split("/")
