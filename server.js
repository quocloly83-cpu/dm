...
async function refreshStoreFromSource(options = {}) {
  const force = !!options.force;
  if (!force && !hasGithubStore()) return keys;
  ...
  try {
    ...
    console.error("Refresh store failed:", err.message);
  }
...
async function saveStore(options = {}) {
  ...
  saveQueue = saveQueue
    .then(async () => {
      if (refreshFirst) {
        await refreshStoreFromSource({ force: true });
      }
      keys = normalizeAllStore(keys);
      const snapshot = JSON.parse(JSON.stringify(keys));
      await persistStoreSnapshot(snapshot);
    })
    .catch((err) => {
      ...
      <div class="homeGlass">
        ...
        <div class="loginPanel">
          ...
          <div style="margin-top:10px">
            <a class="gameBtn" href="/panel" style="width:100%"> Panel</a>
          </div>
